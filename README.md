...
src/lib/sources/*          Source connectors (OSM, CanadaOne, CSV and URL import)
...
- OpenStreetMap through public Overpass endpoints with bounded fallback and exact-radius filtering
- CanadaOne through a dedicated public-directory search connector, capped and rate-limited
- User CSV import
- User-supplied business-owned website URLs

Google Maps, Facebook, Reddit, Kijiji, Yelp pages, and other unsupported
marketplace/social/map-result pages are not scraped. CanadaOne directory email
fields are not considered sendable contact evidence; an address becomes eligible
only when it is published on the business-owned public website (or explicitly
provided by the user in a CSV import).
