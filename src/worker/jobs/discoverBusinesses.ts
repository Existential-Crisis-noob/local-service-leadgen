...
import { CanadaOneConnector } from "@/lib/sources/canadaOne";
...
export async function discoverBusinesses(payload: DiscoverBusinessesPayload) {
  ...
  try {
    if (campaign.connectorType !== "OSM" && campaign.connectorType !== "CANADA_ONE") {
    ...
    const connector = campaign.connectorType === "CANADA_ONE" ? new CanadaOneConnector() : new OsmConnector();
